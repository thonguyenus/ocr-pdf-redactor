...
const GlobalStyle = createGlobalStyle`
  * { box-sizing: border-box; }
  ...
  body { font-family: ui-sans-serif, system-ui, -apple-system, Segoe UI, Roboto, Helvetica, Arial, Noto Sans, Apple Color Emoji, Segoe UI Emoji; color: ${({
    theme,
  }) => (theme as any).colors.text}; background: ${({ theme }) =>
  (theme as any).colors.bg}; }
`;
...
export const App: React.FC = () => {
  ...
  const { pdfProxy, numPages, canvases, isRendering, renderDone, renderError } =
    usePdfRenderer(pdfFile);
  ...
  const runOcrTesseract = useCallback(async () => {
    if (!pdfProxy) return;
    let tries = 0;
    while (canvases.length === 0 && tries < 200) {
      await new Promise((r) => setTimeout(r, 50));
      tries += 1;
    }
    if (!canvases.length) {
      alert("PDF is not fully rendered yet. Please try again shortly.");
      return;
    }
    ...
    try {
      ...
      if (fields.length > 0) {
        ...
        console.table(
          fields.map((f) => ({
            page: f.pageIndex + 1,
            x0: Math.round(f.bbox?.x0 ?? 0),
            y0: Math.round(f.bbox?.y0 ?? 0),
            x1: Math.round(f.bbox?.x1 ?? 0),
            y1: Math.round(f.bbox?.y1 ?? 0),
            text: f.value,
          }))
        );
        console.groupEnd();
      } else {
        console.warn("[OCR] No fields found");
      }
      setOcrFields(fields);
    } catch (e) {
      console.error(e);
  ...
  const runOcrGcv = useCallback(async () => {
    ...
    try {
      ...
      console.error("GCV OCR failed", e);
      alert("Google Vision OCR failed. Please check your API key and access.");
    } finally {
      setIsOcrRunning(false);
    }
  }, [pdfProxy, canvases]);
  ...
  const onExport = useCallback(async () => {
    ...
    const selected = ocrFields.filter(
      (f) => selectedFieldIds.has(f.id) && f.bbox
    );
    if (selected.length === 0) {
      alert("No fields selected to redact.");
      return;
    }
        ...
        <Sidebar>
          <Panel>
            ...
            <Controls
              ...
              onRunOcr={() =>
                ocrEngine === "tesseract" ? runOcrTesseract() : runOcrGcv()
              }
              onExport={onExport}
            ...
            {isRendering && (
              <ProgressWrap>
                <div style={{ fontSize: 12, color: "#475569" }}>
                  Rendering PDF: {renderDone}/{numPages}
                </div>
                <ProgressBar>
                  <ProgressFill
                    $pct={numPages ? (renderDone / numPages) * 100 : 0}
                  />
                </ProgressBar>
              </ProgressWrap>
            )}
