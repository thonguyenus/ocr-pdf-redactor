...
export async function recognizeWithGcv(canvas: HTMLCanvasElement): Promise<OcrField[]> {
  const apiKey = import.meta.env.VITE_GCV_API_KEY as string | undefined;
  if (!apiKey) {
  ...
  for (const p of annotation.pages as any[]) {
    for (const block of p.blocks ?? []) {
      for (const para of block.paragraphs ?? []) {
        ...
        const text = words.map((w: any) => (w.symbols ?? []).map((s: any) => s.text || '').join('')).join(' ').trim();
        if (!text) continue;
        ...
        out.push({
          id: `${pageIndex}-${x0}-${y0}-${x1}-${y1}-${Math.random().toString(36).slice(2)}`,
